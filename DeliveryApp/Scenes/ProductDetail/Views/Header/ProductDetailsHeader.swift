...
final class ProductDetailsHeader: UIView {
    ...
    func configure(with viewData: ProductHeaderViewData, delegate: ProductDetailsHeaderDelegateProtocol) {
        ...
        if let imageUrlString = viewData.images.first?.url, let imageUrl = URL(string: imageUrlString) {
            let placeholderImage = UIImage(systemName: "photo")
            productImageView.sd_setImage(with: imageUrl, placeholderImage: placeholderImage)
        }
    }
}
